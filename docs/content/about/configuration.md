...
| `issuers`                | yes      | List of trusted OIDC issuer URLs. Tokens whose `iss` claim is not in this list are rejected without any network call. Entries ending in `*` are treated as prefix matches (e.g. `https://oidc.example.com/id/*` trusts any issuer under that path). |
