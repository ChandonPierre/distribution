...
| `credentialsendpoint` | no | Full URI of an HTTP endpoint that returns AWS credentials in the [container credentials format](https://docs.aws.amazon.com/sdkref/latest/guide/feature-container-credentials.html). When set, this takes precedence over `accesskey`/`secretkey`. Credentials are cached until their `Expiration` time and then automatically refreshed. |
| `credentialstoken` | no | Static Bearer token sent in the `Authorization` header of each request to `credentialsendpoint`. Ignored if `credentialsendpoint` is not set. |
| `credentialstokenfile` | no | Path to a file whose contents are used as the Bearer token for requests to `credentialsendpoint`. The file is re-read on every credential refresh, so token rotation is picked up automatically. Takes precedence over `credentialstoken`. Ignored if `credentialsendpoint` is not set. |
...
`credentialsendpoint`: (optional) Full URI of an HTTP endpoint implementing the [AWS container credentials protocol](https://docs.aws.amazon.com/sdkref/latest/guide/feature-container-credentials.html). When set, this takes precedence over `accesskey`/`secretkey`. The endpoint must return a JSON response with `AccessKeyId`, `SecretAccessKey`, and optionally `Token` and `Expiration` fields. Credentials are cached in memory until their `Expiration` timestamp and then transparently refreshed on the next S3 operation.

`credentialstoken`: (optional) Static token sent verbatim as the value of the `Authorization` header on requests to `credentialsendpoint`. Only meaningful when `credentialsendpoint` is set.

`credentialstokenfile`: (optional) Path to a file whose contents are sent as the `Authorization` header value on requests to `credentialsendpoint`. The file is re-read on every credential refresh cycle, so rotating tokens — such as Kubernetes projected service account tokens mounted via a `serviceAccountToken` volume — are picked up automatically without restarting the registry. Takes precedence over `credentialstoken`. Only meaningful when `credentialsendpoint` is set.

### Container credentials example (EKS Pod Identity)
...
### Container credentials example (ECS / generic endpoint with static token)
...
storage:
  s3:
    region: us-east-1
    bucket: my-registry-bucket
    credentialsendpoint: http://169.254.170.2/v2/credentials/my-task-id
    credentialstoken: my-static-auth-token
```
