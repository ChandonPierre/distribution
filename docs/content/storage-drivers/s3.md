...
| `maxconnections` | no | Maximum number of idle HTTP connections kept open in the connection pool per host. Higher values reduce connection-establishment overhead under concurrent load. Defaults to `10`. |
...
`maxconnections`: (optional) The size of the HTTP connection pool used for S3 API calls. Each registry process maintains up to this many idle connections per S3 host. Increasing this value reduces TCP handshake overhead under high concurrency (e.g. many parallel layer uploads or downloads). Defaults to `10`. Set higher (e.g. `50`–`200`) for high-throughput deployments, keeping in mind that each idle connection consumes a small amount of memory and a file descriptor.
